...
import (
	dto2 "MarketPulse/internal/server/dto"
	"context"
...
type candleService interface {
	GetHistoricalCandles(ctx context.Context, request *dto2.GetCandlesRequest) (*dto2.CandleHistoryResponse, error)
...
func (c *candleController) GetHistoricalCandles(ctx *gin.Context) {
	var req dto2.GetCandlesRequest

	if err := ctx.ShouldBindQuery(&req); err != nil {
		ctx.JSON(http.StatusBadRequest, dto2.APIResponse{
	...
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, dto2.APIResponse{
	...
	ctx.JSON(http.StatusOK, dto2.APIResponse{
		Code:    http.StatusOK,
		Message: "Success",
		Data:    candleHistory,
	})
